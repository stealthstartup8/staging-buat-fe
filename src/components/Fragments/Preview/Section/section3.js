...
const PreviewSection3 = ({
	...
	return (
		<SectionWrapper
			section={section}
			className="z-20 w-screen h-screen flex flex-col md:flex-row-reverse  min-h-screen md:h-screen relative"
			video={false}
		>
			<div className="w-full md:w-1/2">
				...
				>
					{value_component.components.map((item, index) => (
						<Fragment key={index}>
							<SwiperSlide>
								{value_component.components[index].background_image !== "" && (
									<div
										style={{
			...
			<div
				...
				className="z-20 w-full md:w-1/2 flex flex-col justify-center md:flex-col-reverse  lg:p-8 p-4 relative md:gap-12  bg-[image:var(--image-url)] bg-no-repeat bg-cover h-full bg-center"
			>
				...
				<div>
					<div className="mb-6 container w-fit">
						<Label
						...
						<Tagline
							...
							className={`lg:w-6/12 min-w-full mt-4 `}
							idComponent={idComponent}
							sectionComponentHero={sectionComponentHero}
							editable={editable}
						/>
