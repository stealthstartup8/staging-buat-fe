	...
	return (
		<React.Fragment>
			...
			>
				...
				<MenuList
					className={`hidden max-w-screen-xl rounded-xl outline-none lg:block border-none p-0`}
				>
					<ul
						style={{
							backgroundColor: `${navigation_style.item.background_color}`,
						}}
						className="gap-x-2 outline-none outline-none"
					>
...
const PreviewNavbar5 = ({
	...
	navigation_menu.item.forEach((item, index) => (index < 4 ? rightMenu.item.push(item) : null));

	navigation_menu.item.forEach((item, index) => (index > 3 ? bottomMenu.item.push(item) : null));
	...
	return (
		...
		>
			{(!editable || show_navbar) && (
				...
				>
					<div className="container mx-auto flex items-center justify-between text-blue-gray-900 w-[100%]">
						<div
							onClick={
								editable == true
									? (e) => {
					...
					{bottomMenu.item.length != 0 && (
						<div className="hidden lg:block ">
							<div className="flex w-[100vw] justify-center">
								<NavList
									navigation_menu={bottomMenu}
									navigation_font={navigation_font}
									navigation_style={navigation_style}
									navigation_button={navigation_button}
									editable={editable}
