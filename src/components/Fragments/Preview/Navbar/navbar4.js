...
function NavList({ navigation_menu, navigation_font, navigation_style, navigation_button, editable }) {
	...
	return (
		<List className="justify-center mt-4 mb-6 p-0 lg:mt-0 lg:mb-0 lg:flex-row lg:p-1 gap-2 min-w-0 items-center">
...
const PreviewNavbar4 = ({
	...
	navigation_menu.item.forEach((item, index) => (index % 2 !== 0 ? rightMenu.item.push(item) : null));

	navigation_menu.item.forEach((item, index) => (index % 2 === 0 ? leftMenu.item.push(item) : null));
	...
	return (
		...
		>
			{(!editable || show_navbar) && (
				...
				>
					<div className="container mx-auto flex items-center justify-between text-blue-gray-900 w-[100%] relative">
						<div className="hidden lg:block lg:ml-6">
							<NavList
								navigation_menu={leftMenu}
						...
						<div
							...
							className="lg:absolute lg:left-1/2 lg:transform lg:-translate-x-1/2 lg:flex lg:items-center gap-2 cursor-pointer lg:justify-center lg:mb-2"
						>
