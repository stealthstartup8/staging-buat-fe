...
import {
	...
	textColor,
	textDecoration,
} from "@store/footer/fontSlice";
import Button from "@/components/Elements/Button";

const FontFooterSection = () => {
	...
	const footerFont = useSelector((state) => state.persistedReducer.footerFontSlice.item);
	...
	return (
		<div className="font-section overflow-y-auto content-scrollbar mt-4 pr-1 ">
			<div>
				...
				<div className="bg-[#F5F5F5] rounded-md p-2 mt-2 text-sm leading-tight">
					<div className="pt-2">
						...
						>
							{fontList.map((font, index) => (
								<option
									key={index}
									value={font.name}
									selected={footerFont.font_style == font.name && true}
								>
									{font.name == ""
										? "Default"
						...
						<div className={`px-2 mt-4 mb-4 grid grid-cols-4 gap-1 mt-2`}>
							<Button
								...
								className={` border border-1 rounded-md py-1 ${
									footerFont.bold ? "bg-[#082691] text-white" : "bg-white text-[#082691]"
								}`}
							>
								<b>B</b>
							</Button>
							<Button
								...
								className={` border border-1 rounded-md py-1 ${
									footerFont.italic ? "bg-[#082691] text-white" : "bg-white text-[#082691]"
								}`}
							>
								<i>I</i>
							</Button>
							<Button
								...
								className={`underline  border border-1 rounded-md py-1 ${
									footerFont.text_decoration == "underline"
							...
							<Button
								...
								className={` border border-1 rounded-md py-1 ${
									footerFont.text_decoration == "line-through"
										? "bg-[#082691] text-white"
										: "bg-white text-[#082691]"
								}`}
							>
...
export default FontFooterSection;
