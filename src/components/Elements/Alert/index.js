const Alert = ({ background, color, message, setIsFail }) => {
	return (
		<>
			<div
				id="alert-border-2"
				className={`flex items-center p-4 mb-4 text-${color}-800 border-t-4 border-${background}-300 bg-${color}-50  `}
				role="alert"
			>
				<svg
				...
				<div className="ms-3 text-sm font-medium">
					<span>{message != "" ? message : "Error Login!"}</span>
				</div>
				<button
					type="button"
